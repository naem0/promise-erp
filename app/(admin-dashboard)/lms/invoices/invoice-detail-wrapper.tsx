...
export async function InvoiceDetailWrapper({
  ...
  try {
    ...
    console.error(`Error loading admin invoice wrapper for ID `, error);
    if (error instanceof Error) {
      return <ErrorComponent message={error.message} />;
    }
    return <ErrorComponent message="Failed to load invoice details." />;
  }
  ...
  if (!invoice) {
    return (
      <NotFoundComponent title="Invoice" message={response?.message || " Not Found"} />
    );
  }
  ...
  return (
    <div className="p-6 space-y-6 print:p-0">
      ...
      <InvoiceStudentCard student={invoice?.user} invoiceId={invoice?.invoice_no} />

      {/* Row 1: Course Info & Timeline */}
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-4 xl:gap-6">
        <InvoiceCourseInfo course={invoice?.batch} branchName={invoice?.branch_name} enrollmentDate={invoice?.enrollment_date} />
