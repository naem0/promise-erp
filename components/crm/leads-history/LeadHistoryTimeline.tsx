      ...
      {histories.map((history) => (
        <div key={history.id} className="relative flex items-start group">
          <div className={`flex items-center justify-center w-10 h-10 rounded-full ${getIconBg(history.status_text)} shrink-0 z-10 shadow-sm border border-white`}>
            {getIcon(history.type, history.status_text)}
          </div>
            ...
            <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center text-blue-600">
                    {history.type === 1 && <Phone className="w-4 h-4 mr-2" />}
                    {history.type === 2 && <MessageSquare className="w-4 h-4 mr-2 text-orange-600" />}
                    {history.type === 3 && <Mail className="w-4 h-4 mr-2 text-sky-600" />}
                    {history.type === 4 && <MessageCircle className="w-4 h-4 mr-2 text-primary" />}
                </div>
              </div>
