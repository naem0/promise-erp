...
const TodayFollowUpsData = async ({
  searchParams,
}: {
  ...
  const queryStr = Object.keys(cleanParams).length
  ...
  if (!activities.length) {
    return (
      <NotFoundComponent
        message={results?.message || "No follow-up leads scheduled for today."}
      />
    );
  }
              ...
              <TableRow key={activity?.id}>
                ...
                <TableCell className="text-center">
                  <PermissionGuard requiredPermission="create-lead-activities">
                    <Link
                      href={`/crm/lead-activities/${activity?.lead_id || activity?.id}/manage${queryStr}${queryStr ? "&" : "?"}from_page=/crm/today-follow-ups`}
...
export default TodayFollowUpsData;
