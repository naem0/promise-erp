...
import { getCRMOldLeadReports, CRMOldLeadReportsItem } from "@/apiServices/crmLeadReportsService";
import { useSearchParams } from "next/navigation";
import { toast } from "sonner";

interface ExportCRMOldLeadReportsButtonProps {
  fileName?: string;
...
function buildRows(data: CRMOldLeadReportsItem[], page: number = 1, perPage: number = data.length) {
...
export default function CRMOldLeadReportsExportButton({
  fileName = "crm-old-lead-reports",
  data: providedData,
  page = 1,
  perPage = 15,
}: ExportCRMOldLeadReportsButtonProps) {
  ...
  const fetchAllData = async () => {
    let allData: CRMOldLeadReportsItem[] = [];
    let currentPage = 1;
    let lastPage = 1;
    let hasError = false;
    ...
    do {
      ...
      try {
        const res = await getCRMOldLeadReports(params);
        if (res?.success) {
  ...
  const exportExcel = () => {
    startExporting(async () => {
      ...
      utils.book_append_sheet(wb, ws, "Old Leads Report");
