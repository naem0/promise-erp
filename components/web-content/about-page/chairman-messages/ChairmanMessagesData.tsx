...
import { ChairmanMessage, getChairmanMessages } from "@/apiServices/chairmanMessagesService";
import DeleteChairmanMessageButton from "./DeleteChairmanMessageButton";
...
const ChairmanMessagesData = async ({
  searchParams,
}: {
  ...
  let results;
  try {
    results = await getChairmanMessages(params);
  } catch (error: unknown) {
  ...
  const items = results?.data?.chairman_messages || [];
  const paginationData = results?.data?.pagination;

  if (!items.length) {
    return (
      <NotFoundComponent message={results?.message || "No Management messages found."} />
    );
  }
        ...
        <Table>
          <TableHeader className="bg-slate-50">
            <TableRow>
              ...
              <TableHead className="font-semibold min-w-[300px]">Profile</TableHead>
              <TableHead className="font-semibold min-w-[200px]">Message Title</TableHead>
          ...
          <TableBody>
            {items.map((item: ChairmanMessage, index: number) => (
              <TableRow key={`${item?.id}-${index}`} className="hover:bg-slate-50/50 transition-colors">
                ...
                <TableCell className="text-center">
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="center">
                      <PermissionGuard requiredPermission="edit-chairman-messages">
                        <DropdownMenuItem asChild>
                          <Link
                            href={`/web-content/about-page/chairman-messages/${item?.id}/edit`}
                      ...
                      <PermissionGuard requiredPermission="delete-chairman-messages">
                        <DropdownMenuItem asChild>
                          <DeleteChairmanMessageButton id={item?.id} />
                        </DropdownMenuItem>
                      </PermissionGuard>
                    </DropdownMenuContent>
                  </DropdownMenu>
                ...
                <TableCell>
                  <div className="flex items-center gap-3">
                    <div className="relative w-12 h-12 rounded-md overflow-hidden border border-slate-100 flex-shrink-0">
                      <Image
                        src={(item.chairman_image && typeof item.chairman_image === "string" && item.chairman_image.trim() !== "") ? item.chairman_image : "/images/placeholder.png"}
                        alt={item.name}
                ...
                <TableCell>
                  <div className="flex flex-col min-w-0 gap-0.5">
                    <span className="font-medium text-slate-800 truncate" title={item?.message_title}>
                      {truncate(item?.message_title, 40)}
                    </span>
                    <span className="text-xs text-slate-500 truncate" title={item?.message_content}>
                       {truncate(item?.message_content, 60)}
                    </span>
                  </div>
                </TableCell>
...
export default ChairmanMessagesData;
