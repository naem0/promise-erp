...
interface GroupItemSearchSelectProps {
  groups?: (SearchGroupItemResult | { id: number; name: string; barcode?: string | null; image?: string | null })[]
  value?: number | string | null
  onValueChange?: (value: string | null) => void
  onGroupItemChange?: (groupItem: SearchGroupItemResult | null) => void
  placeholder?: string
...
export default function GroupItemSearchSelect({
  ...
  const [groupsList, setGroupsList] = useState<SearchGroupItemResult[]>((initialGroups as SearchGroupItemResult[]) || [])
  const [groupsMap, setGroupsMap] = useState<Record<string, SearchGroupItemResult>>({})
  ...
  useEffect(() => {
    if (initialGroups && initialGroups.length > 0) {
      setGroupsList(initialGroups as SearchGroupItemResult[])
      setGroupsMap((prev) => {
        const newMap = { ...prev }
        initialGroups.forEach((g) => {
          if (g?.id != null) {
            newMap[String(g.id)] = g as SearchGroupItemResult
          }
        })
        return newMap
      })
  ...
  const handleSelectChange = (val: string | null) => {
    onValueChange?.(val)
    if (onGroupItemChange) {
      const selected = val ? groupsMap[val] || null : null
      onGroupItemChange(selected)
    }
  }
