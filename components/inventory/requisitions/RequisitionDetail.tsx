...
export default function RequisitionDetail({
  ...
  const isAnySelected = watchedItems.some((item) => item.selected);
  ...
  const onSubmit = async (data: FormValues, action: "Approve" | "Reject") => {
    ...
    // Check if any selected item has approved_qty < 1
    const hasInvalidQty = selectedItems.some((item) => item.approved_qty < 1);
    if (hasInvalidQty) {
      toast.error("Approved Qty cannot be less than 1 for selected items.");
      return;
    }
    ...
    try {
      ...
      if (res?.success) {
        toast.success(res.message || `Requisition items ${action.toLowerCase()}d successfully.`);
        router.refresh();
      } else {
              ...
              <Table>
                ...
                <TableBody>
                  {!requisition?.items || requisition?.items?.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={8} className="text-center py-3.5">
                        No items found.
                      </TableCell>
                    </TableRow>
                  ) : (
