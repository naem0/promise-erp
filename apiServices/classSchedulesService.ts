...
export interface ActiveClassSchedulesResponse {
  success: boolean;
  message: string;
  code: number;
  data: ClassSchedule[];
...
export async function getClassScheduleById(
  ...
  try {
    ...
    if (error instanceof Error) {
      throw new Error(
        error.message || "Failed to fetch class schedule details",
      );
    } else {
...
export async function getActiveClassSchedules(): Promise<ActiveClassSchedulesResponse> {
  const session = await getServerSession(authOptions);
  const token = session?.accessToken;
  ...
  try {
    const res = await fetch(`${API_BASE}/class-schedules/active`, {
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
      },
    ...
    if (error instanceof Error) {
      console.error("Error in getActiveClassSchedules:", error.message);
      throw new Error(error.message);
    } else {
      throw new Error("Failed to get active class schedules");
    }
  }
}

// =======================
// TOGGLE CLASS SCHEDULE STATUS
// =======================

export async function toggleClassScheduleStatus(
  id: number,
): Promise<SingleClassScheduleResponse> {
  const session = await getServerSession(authOptions);
  const token = session?.accessToken;
  ...
  try {
    const res = await fetch(`${API_BASE}/class-schedules/${id}/toggle-status`, {
      method: "PATCH",
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
    ...
    if (error instanceof Error) {
      console.error("Error in toggleClassScheduleStatus:", error);
      throw new Error(
        error.message || "Failed to toggle class schedule status",
      );
    } else {
      throw new Error("Failed to toggle class schedule status");
    }
  }
}
